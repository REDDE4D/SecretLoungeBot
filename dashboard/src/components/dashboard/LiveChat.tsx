...
const renderIcon = (icon: string | { customEmojiId: string; fallback: string }): string => {
...
export function LiveChat() {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload: { text: string; replyToAlias?: string; replyToText?: string } = {
      ...
      const response = await apiClient.post<{ message: string; stats: { sent: number; failed: number; total: number } }>(
        '/chat/send',
        payload
      );
  ...
  const isMediaMessage = (type: string) => {
    return ['photo', 'video', 'audio', 'document', 'voice', 'animation', 'sticker', 'media_group'].includes(type);
  };
        ...
        <div className="space-y-4">
          {messages.length === 0 ? (
            <div className="flex h-full items-center justify-center text-center text-muted-foreground">
              <p>No messages yet. Messages from the Telegram lobby will appear here.</p>
            </div>
          ) : (
            messages.map((msg) => (
              <div key={`${msg.userId}-${msg.id}-${msg.timestamp}`} className="group flex gap-3 rounded-lg border p-3 hover:bg-muted/50 transition-colors">
                  ...
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2 flex-wrap">
                      {getKarmaEmoji(msg.karma) && (
                        <span className="text-sm">{getKarmaEmoji(msg.karma)}</span>
                      )}
                      <span className="font-semibold">{msg.alias}</span>
                      {msg.role && ['admin', 'mod', 'whitelist'].includes(msg.role) && (
                        <span className="text-sm">{getRoleEmoji(msg.role)}</span>
                      )}
                  ...
                  <div className="mt-1 break-words">
                    {msg.text ? (
                      ...
                      <p className="text-sm text-muted-foreground italic">
                        {msg.type === 'media_group' ? '[Media Album]' : `[${msg.type}]`}
                      </p>
                    ) : (
                      <p className="text-sm text-muted-foreground italic">[No text]</p>
                    )}
                  </div>
                </div>
              </div>
      ...
      <div className="border-t p-4">
        ...
        <form onSubmit={handleSendMessage} className="flex gap-2">
          <Input
            ...
            className="flex-1"
          />
          <Button type="submit" disabled={!messageText.trim() || sending || !connected} size="icon">
            {sending ? (
        ...
        <p className="mt-2 text-xs text-muted-foreground">
          Messages sent here will appear in the Telegram lobby as coming from your admin account.
        </p>
      </div>
    </div>
  );
