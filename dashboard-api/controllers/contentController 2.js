...
export const updateFilter = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const filter = await contentService.updateFilter(id, req.body, req.user.userId);
  res.json({ success: true, message: "Filter updated", data: filter });
});

export const deleteFilter = asyncHandler(async (req, res) => {
  const { id } = req.params;
  await contentService.deleteFilter(id, req.user.userId);
...
export const updateInvite = asyncHandler(async (req, res) => {
  ...
  const invite = await contentService.updateInvite(code, active, req.user.userId);
  res.json({ success: true, message: "Invite updated", data: invite });
});

export const deleteInvite = asyncHandler(async (req, res) => {
  const { code } = req.params;
  await contentService.deleteInvite(code, req.user.userId);
...
export const deletePin = asyncHandler(async (req, res) => {
  const { id } = req.params;
  await contentService.deletePin(parseInt(id), req.user.userId);
...
export const deleteScheduled = asyncHandler(async (req, res) => {
  const { id } = req.params;
  await contentService.deleteScheduledAnnouncement(id, req.user.userId);
