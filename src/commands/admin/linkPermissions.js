...
async function setLinkPermission(ctx, args, adminId) {
  ...
  try {
    ...
    if (user.canPostLinks === enable) {
      ...
      return ctx.reply(
        `ℹ️ ${escapeHTML(alias)} ${status} link posting permission.`
      );
    }
    ...
    await ctx.reply(
      `❌ Error: ${escapeHTML(err.message || "Could not resolve user.")}`
    );
  }
}
...
async function listLinkPermissions(ctx, page = 1) {
  try {
    ...
    const userLines = users.map((user) => {
      ...
      const roleIcon =
        user.role === "owner"
          ...
          ? "🛡️"
          : "";
      return `${statusIcon} ${roleIcon} ${
        user.icon?.fallback || "👤"
      } ${escapeHTML(user.alias || "Unknown")}`;
    });
    ...
    if (page > 1) {
      row.push({
        text: "◀️ Previous",
        callback_data: `linkperm_page:${page - 1}`,
      });
    }
